...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("费率基准校准: 武警特勤支队2026年无人机保险采购标书")
...
📌 <strong>v0.2 更新</strong>：费率基准基于武警特勤支队2026年无人机保险采购标书校准
（预算¥18万 / 16架无人机 + 三者险≥200万 / 整体费率≈0.55%）
</div>
""", unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown("""
    **版本: v0.2 — 规则引擎（基于真实标书校准）**

    **费率基准**: 武警特勤支队2026年无人机保险采购标书
    - 采购预算 ¥180,000 / 16架无人机
    - 综合费率 ≈ **0.55%**（包含机身险+三者险）
    - 三者险单机保额 ≥ **200万元**
    """)
    ...
    | 历史理赔 | ~20% | 近3年出险次数 |
    | 保额配置 | ~12% | 保额/设备价值比 |
    | 三者险风险 | ~8% | 三者险保额对应的风险敞口 |
    | 机队折扣 | ~-5% | 批量投保折扣 |
    """)

    st.markdown("---")
    st.markdown("**政策背景（2026年）**")
    st.markdown("""
    - 🔴 2026.01.01 无人机"黑飞"正式入法
    - 🔴 2026.02.13 三部门：无人驾驶航空器责任保险**强制投保**
    - 🔴 2026.03 40余家险企推出约 **180款** 低空保险产品
    - 🟢 **机会**：数据不通、风险复杂是行业公认最大障碍
    """)

st.markdown("---")
st.markdown("*晶世科保 · 无人机保险AI引擎 v0.2 · 费率校准自武警特勤支队2026年标书*")
